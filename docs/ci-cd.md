    ...
    steps:
      - name: Verify PolicyChecks claims
        ...
        run: |
          ...
          const failures = data.claims.filter((claim) => claim.result !== 'enabled');
...
This treats both `disabled` and `unknown` as non-enabled. A looser gate could block only `disabled` and allow `unknown`, but that should be an explicit repository policy decision.
