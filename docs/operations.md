...
Expected cold-cache request cost for one repository is bounded: installation lookup and installation-token creation, repository metadata when needed, immutable releases, Actions permissions, and vulnerability-alerts status. Warm-cache badge/proof requests should make zero GitHub API calls.
